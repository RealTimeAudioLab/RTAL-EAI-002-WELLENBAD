...
// Arpeggiator
// ================================================================
// BUILD0047 ARPSTATE1: physical key state is separate from ARP HOLD/latch.
static uint8_t arpHeldInputNotes[ARP_MAX_NOTES];
static uint8_t arpHeldInputVelocity[ARP_MAX_NOTES];
static uint8_t arpHeldInputCount = 0;

void arpTrackInputNoteOn(uint8_t note, uint8_t vel) {
  for (uint8_t i=0;i<arpHeldInputCount;++i) if (arpHeldInputNotes[i]==note) { arpHeldInputVelocity[i]=vel; return; }
  if (arpHeldInputCount < ARP_MAX_NOTES) { arpHeldInputNotes[arpHeldInputCount]=note; arpHeldInputVelocity[arpHeldInputCount]=vel; ++arpHeldInputCount; }
}
void arpTrackInputNoteOff(uint8_t note) {
  for (uint8_t i=0;i<arpHeldInputCount;++i) if (arpHeldInputNotes[i]==note) {
    for (uint8_t j=i;j+1<arpHeldInputCount;++j) { arpHeldInputNotes[j]=arpHeldInputNotes[j+1]; arpHeldInputVelocity[j]=arpHeldInputVelocity[j+1]; }
    --arpHeldInputCount; return;
  }
}
void arpClearInputState() { arpHeldInputCount = 0; }

// B0063F: authoritative physical-key state query for the audio-side
// lost-NoteOff safety guard. This is updated in MidiTask before the
// corresponding audio event is queued.
bool arpInputNoteHeld(uint8_t note) {
  for (uint8_t i = 0; i < arpHeldInputCount; ++i) {
    if (arpHeldInputNotes[i] == note) return true;
  }
  return false;
}
...
void arpTransitionMode(uint8_t oldMode, uint8_t newMode) {
  ...
  if (oldMode == 0 && newMode != 0) {
    ...
    arpCount=0; arpIndex=0; arpDir=1; currentArpNote=255;
    for (uint8_t i=0;i<arpHeldInputCount;++i) arpAddNote(arpHeldInputNotes[i], arpHeldInputVelocity[i]);
  } else if (oldMode != 0 && newMode == 0) {
    arpAllNotesOff(); arpCount=0; arpIndex=0; arpDir=1; currentArpNote=255;
    for (uint8_t i=0;i<arpHeldInputCount;++i) queueAudioEvent(AE_NOTE_ON, arpHeldInputNotes[i], arpHeldInputVelocity[i]);
  }
}
...
// B0074: silence the currently sounding ARP note without destroying the
// latched HOLD pool, physical held-note bookkeeping, index or clock phase.
// ENABLE/DISABLE and MUTE use this path so a held Part can be brought back
// into a performance at the next timing grid point.
void multiArpSilencePart(uint8_t partIndex) {
  if (partIndex >= RTAL_MULTI_PART_COUNT) return;
  MultiArpState &st = multiArp[partIndex];
  if (st.currentNote != 255) {
    queueAudioEvent(AE_ARP_NOTE_OFF, st.currentNote, 0, partIndex,
...
void multiArpResetPart(uint8_t partIndex, bool clearInputNotes) {
  ...
  if (clearInputNotes) { st.count = 0; st.heldCount = 0; }
  st.index = 0;
  st.dir = 1;
...
void multiArpTrackInputNoteOn(uint8_t partIndex, uint8_t note, uint8_t vel) {
  if (partIndex >= RTAL_MULTI_PART_COUNT) return; MultiArpState &st=multiArp[partIndex];
  for (uint8_t i=0;i<st.heldCount;++i) if (st.heldNotes[i]==note) { st.heldVelocity[i]=vel; return; }
  if (st.heldCount < ARP_MAX_NOTES) { st.heldNotes[st.heldCount]=note; st.heldVelocity[st.heldCount]=vel; ++st.heldCount; }
}
void multiArpTrackInputNoteOff(uint8_t partIndex, uint8_t note) {
  if (partIndex >= RTAL_MULTI_PART_COUNT) return; MultiArpState &st=multiArp[partIndex];
  for (uint8_t i=0;i<st.heldCount;++i) if (st.heldNotes[i]==note) {
    for (uint8_t j=i;j+1<st.heldCount;++j) { st.heldNotes[j]=st.heldNotes[j+1]; st.heldVelocity[j]=st.heldVelocity[j+1]; }
    --st.heldCount; return;
  }
}
static void multiArpSeedFromHeld(uint8_t partIndex) {
  MultiArpState &st=multiArp[partIndex]; st.count=0; st.index=0; st.dir=1; st.nextInternalStepUs=0; st.midiClockCounter=0;
  for (uint8_t h=0; h<st.heldCount && st.count<ARP_MAX_NOTES; ++h) {
    int pos=st.count; while (pos>0 && st.notes[pos-1]>st.heldNotes[h]) { st.notes[pos]=st.notes[pos-1]; st.velocity[pos]=st.velocity[pos-1]; --pos; }
    st.notes[pos]=st.heldNotes[h]; st.velocity[pos]=st.heldVelocity[h]; ++st.count;
  }
  if (st.count>0 && getClockSource()==0) { uint32_t nowUs=micros(); multiArpStepNow(partIndex); st.nextInternalStepUs=nowUs+multiArpStepMs(partIndex)*1000UL; }
}
...
void multiArpSetParam(uint8_t partIndex, uint8_t paramId, uint8_t value) {
  ...
  if (paramId == P_ARP_MODE && oldValue != value) {
    MultiArpState &st = multiArp[partIndex];
    if (oldValue == 0 && value != 0) {
      // WB0064: when ARP is enabled while a chord is already held, do not
      // leave the former direct chord voices running through their release while
      // new ARP voices start. With the 4-pole filter that temporary overlap can
      // exhaust the 7-voice realtime budget. Hard-stop the old direct voices first.
      queueAudioEvent(AE_PART_ALL_SOUND_OFF, 0, 0, partIndex, part.midiChannel);
      if (st.currentNote != 255) queueAudioEvent(AE_ARP_NOTE_OFF, st.currentNote, 0, partIndex, part.midiChannel);
      st.currentNote=255; st.gatePending=false; st.gateOffUs=0;
      multiArpSeedFromHeld(partIndex);
    } else if (oldValue != 0 && value == 0) {
      if (st.currentNote != 255) queueAudioEvent(AE_ARP_NOTE_OFF, st.currentNote, 0, partIndex, part.midiChannel);
      st.currentNote=255; st.gatePending=false; st.gateOffUs=0; st.count=0; st.index=0; st.dir=1; st.nextInternalStepUs=0; st.midiClockCounter=0;
      for (uint8_t i=0;i<st.heldCount;++i) queueAudioEvent(AE_NOTE_ON, st.heldNotes[i], st.heldVelocity[i], partIndex, part.midiChannel);
    } else { st.index=0; st.dir=1; st.midiClockCounter=0; }
