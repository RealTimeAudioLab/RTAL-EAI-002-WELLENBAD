...
// B0074: live Part switching is intentionally different from a structural
// Part reset. Preserve HOLD-ARP notes and sequencer position, but guarantee
// that no current voice or pending ARP gate remains audible.
void silenceMultiPartForPerformance(uint8_t partIndex, const char *reason) {
  if (partIndex >= RTAL_MULTI_PART_COUNT) return;
  multiArpSilencePart(partIndex);
  partAllSoundOffCount++;
  queueAudioEvent(AE_PART_ALL_SOUND_OFF, 0, 0, partIndex,
                  multiParts[partIndex].midiChannel);
  Serial.print(F("MULTI PART SILENCE | P")); Serial.print(partIndex + 1);
  Serial.print(F(" | ")); Serial.println(reason ? reason : "PERFORMANCE");
}
...
void handleNoteOn(byte ch, byte note, byte vel) {
  ...
  if (multiModeActive()) {
    ...
    for (uint8_t part = 0; part < RTAL_MULTI_PART_COUNT; ++part) {
      ...
      if (note < multiParts[part].keyLow || note > multiParts[part].keyHigh) continue;
      matched = true;
      multiArpTrackInputNoteOn(part, note, vel);
...
void handleNoteOff(byte ch, byte note, byte vel) {
  if (multiModeActive()) {
    for (uint8_t part = 0; part < RTAL_MULTI_PART_COUNT; ++part) {
      if (!multiPartMatchesMidiChannel(part, ch)) continue;
      if (note < multiParts[part].keyLow || note > multiParts[part].keyHigh) continue;
      multiArpTrackInputNoteOff(part, note);
      if (multiArpParam(part, P_ARP_MODE) != 0) {
...
static void processMidiCcEvent(byte ch, byte cc, byte val) {
  ...
  // ==============================================================
  // Build 0044a MIDI/GUI sync fix:
  // BANKED + LEARN must use the real PERFORMANCE context, not the internal
  // multi-engine state. Normal F/U performances are rendered internally via
  // multiParts[0], but their authoritative editable/savable Program is
  // currentProgram. Routing them through the Multi path made CC changes audible
  // while the GUI and STORE path kept the old value.
  //
  // In a real Mxxx Multi, controller routing remains channel/Part-aware and
  // layered Parts on the same MIDI channel intentionally receive the same CC.
  // ==============================================================
...
void handlePC(byte ch, byte program) {
  ...
  // The MidiTask only publishes a 2-byte request.  No SD, FAT, Program or
  // Multi load is ever executed on its 4096-byte stack.  A one-slot mailbox
  // deliberately coalesces bursts: for performance patch changes only the
  // newest Program Change matters, and avoiding a queue removes permanent
  // xQueueReceive()/scheduler traffic from the ControlTask hot idle path.
  const uint8_t slot = program & 0x7F;
  const uint8_t mode = performanceMultiActive() ? 1 : 0;

  portENTER_CRITICAL(&midiProgramMux);
  if (midiProgramMailbox.pending) midiProgramCoalescedCount++;
  midiProgramMailbox.program = slot;
  midiProgramMailbox.multiMode = mode;
  midiProgramMailbox.pending = true;
  portEXIT_CRITICAL(&midiProgramMux);
  midiProgramDeferredCount++;
}

void processDeferredMidiProgramChanges() {
  // Normal case while playing: one volatile byte read and immediate return.
  // This replaces B0073's unconditional xQueueReceive() on every ControlTask pass.
  if (!midiProgramMailbox.pending) return;

  uint8_t slot;
  uint8_t requestedMulti;
  portENTER_CRITICAL(&midiProgramMux);
  if (!midiProgramMailbox.pending) {
    portEXIT_CRITICAL(&midiProgramMux);
    return;
  }
  slot = midiProgramMailbox.program;
  requestedMulti = midiProgramMailbox.multiMode;
  midiProgramMailbox.pending = false;
  portEXIT_CRITICAL(&midiProgramMux);

  // If the user changed SINGLE/MULTI mode between reception and service,
  // discard the stale request instead of loading data into the wrong mode.
  const bool nowMulti = performanceMultiActive();
  if (nowMulti != (requestedMulti != 0)) return;

  if (nowMulti) {
    if (!safeLoadMultiSetup(slot)) {
      Serial.print(F("MIDI PC MULTI LOAD FAILED | M"));
