...
void rebuildWaveMonitorCache(uint8_t table, uint8_t logicalWave, uint8_t yMid, uint8_t height) {
  ...
  for (uint8_t x = 0; x < 128; x++) {
    ...
    int16_t s = readRTALSample(table, logicalWave, sampleIndex);
...
const char* uiParamName(uint8_t p) {
  ...
  if (p == P_LFO_SHAPE) return "LFO Shape";
...
void updateDisplay() {
  ...
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x10_tf);
  u8g2.setCursor(0, 8); u8g2.print("RTAL WELLENBAD");
  ...
  for (int slot = 0; slot < 4; slot++) {
    ...
    if (p == UI_PARAM_SEQ_TABLE_MODE) {
      ...
      u8g2.setCursor(valueX, y);
      u8g2.print(lfoShapeName(getAParam(P_LFO_SHAPE)));
    }
