...
#define RTAL_PROFILE_SAMPLE_DIV 64U
...
void RTALProfiler::endBlock(uint8_t activeVoices) {
  ...
  if (profileThisBlock) {
    ...
    struct SamplePair { Section id; uint32_t us; };
    const SamplePair detail[] = {
      { SECTION_OSC, sampledOscUs },
      { SECTION_FILTER, sampledFilterUs },
      { SECTION_ENVAMP, sampledEnvAmpUs },
      { SECTION_PAN, sampledPanUs },
      { SECTION_MIX, sampledMixUs }
    };
    for (const auto &d : detail) {
      Counter &c = section[d.id];
      c.totalUs += d.us;
      c.calls++;
      if (d.us > c.maxUs) c.maxUs = d.us;
    }
  }
...
void RTALProfiler::print() {
  ...
  Serial.printf("FX %%        : %6.2f\n", percent(snapSection[SECTION_FX].totalUs,
                                                   sampledAudioReferenceUs));
  Serial.printf("OSC %%       : %6.2f | max %4lu us\n", percent(snapSection[SECTION_OSC].totalUs, sampledAudioReferenceUs), snapSection[SECTION_OSC].maxUs);
  Serial.printf("FILTER %%    : %6.2f | max %4lu us\n", percent(snapSection[SECTION_FILTER].totalUs, sampledAudioReferenceUs), snapSection[SECTION_FILTER].maxUs);
  Serial.printf("ENV/AMP %%   : %6.2f | max %4lu us\n", percent(snapSection[SECTION_ENVAMP].totalUs, sampledAudioReferenceUs), snapSection[SECTION_ENVAMP].maxUs);
  Serial.printf("PAN %%       : %6.2f | max %4lu us\n", percent(snapSection[SECTION_PAN].totalUs, sampledAudioReferenceUs), snapSection[SECTION_PAN].maxUs);
  Serial.printf("MIX/OUT %%   : %6.2f | max %4lu us\n", percent(snapSection[SECTION_MIX].totalUs, sampledAudioReferenceUs), snapSection[SECTION_MIX].maxUs);
