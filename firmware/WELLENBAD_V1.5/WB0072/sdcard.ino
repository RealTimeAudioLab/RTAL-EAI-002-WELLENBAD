...
static inline uint8_t currentLogicalWaveIndex() {
  return (uint8_t)(((uint16_t)getAParam(P_WAVE_POS) * (RTAL_WT_WAVES - 1) + 63) / 127);
}
...
void requestMultiSdWavetableCache(uint8_t partIndex, uint8_t sd) {
  if (partIndex >= RTAL_MULTI_PART_COUNT || sd >= sdWavetableCount) return;
  if (multiSdWtCacheReady[partIndex] && multiSdWtCacheSlot[partIndex] == sd) {
    multiSdWtCacheHitCount++;
    return;
  }
  requestedMultiSdWtSlot[partIndex] = sd;
  multiSdWtCacheRequestCount++;
}

void requestMultiPartCurrentWavetable(uint8_t partIndex) {
  if (partIndex >= RTAL_MULTI_PART_COUNT) return;
  uint8_t table = multiParts[partIndex].program.param[P_WAVETABLE];
...
void processSdWavetableCacheRequest() {
  // Multi requests have priority. Process at most one SD file per ControlTask
  // visit so UI/MIDI work is not held off by a four-file Multi restore.
  static uint8_t rr = 0;
  if (multiModeActive()) {
    for (uint8_t n = 0; n < RTAL_MULTI_PART_COUNT; ++n) {
      uint8_t p = (uint8_t)((rr + n) % RTAL_MULTI_PART_COUNT);
      int16_t sd = requestedMultiSdWtSlot[p];
      if (sd < 0) continue;
      requestedMultiSdWtSlot[p] = -1;
      rr = (uint8_t)((p + 1) % RTAL_MULTI_PART_COUNT);
      if (sd < sdWavetableCount && loadSdWavetableIntoPartCache((uint8_t)sd, p)) {
        multiSdWtCacheLoadCount++;
        Serial.print("MULTI SD WT LOAD | P"); Serial.print(p + 1);
        Serial.print(" <- SD"); Serial.print(sd); Serial.print(" ");
        Serial.println(sdWavetableNames[sd]);
      } else {
        multiSdWtCacheLoadFailCount++;
        Serial.print("MULTI SD WT FAIL | P"); Serial.print(p + 1);
  ...
  requestedSdWtSlot = -1;
  if (sd >= sdWavetableCount) return;
  if (ensureSdWavetableCached((uint8_t)sd)) sdWtCacheLoadCount++;
  else sdWtCacheLoadFailCount++;
}
...
void loadSdWavetables() {
  ...
  if (!dir || !dir.isDirectory()) {
    Serial.println("No /RTALWT directory on SD - using internal wavetables only");
    if (dir) dir.close();
    return;
  }
  ...
  // Allocate audio-rate SRAM only when SD wavetables actually exist.
  if (sdWavetableCount > 0) {
    bool cacheOk = multiModeActive() ? allocateMultiSdWtCaches() : allocateSdWtCaches();
    if (cacheOk && multiModeActive()) {
      Serial.print("Multi SD WT cache: 4 x ");
      Serial.print(SD_WT_CACHE_BYTES / 1024);
      Serial.print(" kB internal SRAM | free heap ");
...
void ensureSdDirectories() {
  ...
  if (!SD.exists(SD_MULTI_DIR)) SD.mkdir(SD_MULTI_DIR);
}
...
bool loadFirstPresetFromSD() {
  ...
  while (true) {
    ...
    if (!entry.isDirectory()) {
      ...
      size_t l = strlen(n);
      if (l >= 5 && strcasecmp(n + l - 5, ".RTAL") == 0) {
        char path[96];
