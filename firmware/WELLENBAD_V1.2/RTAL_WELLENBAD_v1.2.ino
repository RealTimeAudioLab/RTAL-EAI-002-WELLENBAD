...
static inline void readAudioSnapshot(AudioSnapshot &a) {
  ...
  a.filterAftertouchOffset = ((int32_t)channelAftertouch * a.aftertouchFilter) >> 7;
...
void loadBrowserProgram() {
  ...
  // The new UI returns to its performance dashboard after a successful load.
  if (rtalUiIsActive()) {
    rtalUiHandleButton(8, false); // BACK semantics = HOME
    char message[20];
    snprintf(message, sizeof(message), "LOADED %c%03u",
...
static inline int16_t readRTALSampleRouted(uint8_t table, int16_t sd, uint8_t wave, uint8_t sample) {
...
static inline int16_t readWaveAt(uint32_t &phase, uint32_t inc, int32_t wavePos, uint8_t table, int16_t sd, uint16_t pitchFactorQ12) {
  uint8_t si = phase >> 24;
  ...
  if (sd < 0 && table < RTAL_WT_TABLES) {
    ...
    // SD override path.
    a0 = readRTALSampleRouted(table, sd, wa, si);
    a1 = readRTALSampleRouted(table, sd, wa, (si + 1) & WT_MASK);
    b0 = readRTALSampleRouted(table, sd, wb, si);
    b1 = readRTALSampleRouted(table, sd, wb, (si + 1) & WT_MASK);
  }
...
static inline int16_t readWaveAtFast(uint32_t &phase, uint32_t inc, int32_t wavePos, uint8_t table, int16_t sd, uint16_t pitchFactorQ12) {
  uint8_t si = phase >> 24;
  uint8_t sf = (phase >> 16) & 0xFF;
...
static inline int16_t oscRead(Voice &v, const AudioSnapshot &a, bool fastPoly) {
  ...
  if (fastPoly) {
    ...
    s = readWaveAtFast(v.phaseA, inc, wavePos + waveOffset, wtTable, wtSd, a.pitchBendFactorQ12);
    v.phaseB += applyPitchBendFast(v.incB, a.pitchBendFactorQ12);
  } else {
    int32_t oscA = readWaveAt(v.phaseA, base, wavePos, wtTable, wtSd, a.pitchBendFactorQ12);
    int32_t waveBPos = wavePos + (a.oscBOffset << 8);
    int32_t oscB = readWaveAt(v.phaseB, v.incB, waveBPos, wtTable, wtSd, a.pitchBendFactorQ12);
...
static inline int16_t renderVoice(Voice &v, const AudioSnapshot &a, bool fastPoly) {
  ...
  int32_t amp = (smoothVolume.current >> 8) * v.runtime.velocityAmpFactor;
